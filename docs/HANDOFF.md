...
- v0.2 기반: `SourceId`를 token/AST/IR span에 전파하고 여러 파일을 구분하는
  ...
  원본 source span을 유지한다. Project loader와 package/import/visibility grammar는
  아직 구현하지 않았다.
...
1. `docs/todo-v02-project-model/open-questions.md`의 추천안을 승인하거나 수정한다.
2. 승인된 package/import/visibility, manifest, source layout을 `SPEC.md`에 반영한다.
3. manifest와 deterministic project source discovery를 구현한다.
