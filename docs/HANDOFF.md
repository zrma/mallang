...
검증했다. B1 P175c2a는 core statement와 Pratt/postfix expression parser를 Rust
Stage0 oracle에 대해 strict C, sanitizer, allocation accounting까지 differential
검증했다. 다음 활성 slice는 P175c2b control-flow/function-literal/match parser이며,
public language 또는 standard-library addition은 representative compiler blocker가
확인되기 전에는 포함하지 않는다.
