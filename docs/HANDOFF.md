...
분리된 default-transition gate는 public standalone `build`/`run`의 generated C와 native
behavior를 비교하며, 미이관 command와 missing `mlgc`가 Stage0로 fallback하지 않는지
검증한다. No-flag default와 release archive는 아직 Stage0이며 project/diagnostic surface는
P179b, formatter/test/native workflow는 P179c, packaged default switch는 P179d에 남아 있다.
