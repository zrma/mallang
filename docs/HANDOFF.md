...
- 소스 확장자: `.mlg`
- CLI: `mlg`
- 현재 구현: token model, hand-written lexer, AST, parser, semantic checker, ownership-lite move/borrow checks, same-call borrow conflict checks, typed IR, first native subset C backend, `mlg check`, `mlg build`
- 아직 없음: statement-spanning borrow lifetimes, full C backend, `if`/`match`
...
1. `if` expression parser/type checker 추가
2. `Option[T]` / `Result[T, E]` surface 설계
