...
1. fixed-size array semantic/type checking과 move-only ownership rule 추가
2. fixed-size array typed IR/C struct-wrapper layout 추가
3. array-only `for i, value := range values { ... }` parser/semantic/backend 추가
4. `[N]T{...}` array literal backend smoke를 semantic/IR 이후에 추가
