...
일치한다. P177b4는 three-clause init/condition/post, nested `continue` 뒤 post 실행,
loop-exit cleanup과 array/slice range를 추가했다. Range temporary owner는 normal exit와
`break`까지 유지되고 loop-local cleanup은 `continue` 전에 실행된다. Binary prelude도
...
source 846개 normalized IR function이 일치한다. P177b5 경계에서는 slice append와
dynamic owned string이 남아 있었다. P177b6는
direct/field/indexed-field `SliceAppend`, field source empty-header reset, capacity growth,
...
P177c1은 `con`/`mut` parameter pointer ABI, borrowed parameter dereference environment,
...
다음 P177c slice다.
P177c3은 checked integer negate/divide/remainder의 overflow/zero guard와 canonical
`Expr.Binary.LogicalAnd`/`LogicalOr` short-circuit dispatch를 추가했다. 확장 scalar fixture와
...
P179a 구현은 internal engine 이름을 `mlgc`, selector를 `--compiler stage0|self`로
고정했다. `--version --verbose`는 driver/compiler/core provenance를 출력하고,
`scripts/build-self-hosted-compiler.sh`는 Stage0 -> Stage1 -> fixed Stage2 graph를 만든다.
분리된 default-transition gate는 public standalone `check`/`build`/`run`의 성공 behavior와
generated C/native behavior를 비교한다. P179b1에서 Mallang의 bounded diagnostic record를
기존 human/JSON schema로 복원하고 semantic rejection과 multi-error parser recovery를
Stage0와 비교하며, malformed protocol과 missing `mlgc`는 fallback 없이 실패한다. No-flag
...
path, cycle의 human/JSON Stage0 parity를 확인한다. P179b2b3은 standalone/project public
`ir`을 `mlgc`로 연결하고, Stage0와 self가 Rust-specific pretty debug 대신 deterministic
`IR|...` normalized view를 동일하게 출력하도록 한다. Host는 record count, source ID,
