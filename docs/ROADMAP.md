...
P154 inventory와 Q1-Q6 결정은 `docs/artifacts/v07-tooling-platforms/feasibility.md` 및
...
`docs/artifacts/v07-tooling-platforms/p156-test-workflow.md`가 소유한다. Contextual
...
`docs/artifacts/v07-tooling-platforms/p157-local-path-dependencies.md`가 소유한다.
...
`docs/artifacts/v07-tooling-platforms/p158-machine-readable-diagnostics.md`가 소유한다.
...
`docs/artifacts/v07-tooling-platforms/p159-release-artifacts-installation.md`가 소유한다. Local
...
`docs/artifacts/v07-tooling-platforms/p160-v07-acceptance.md`가 소유한다. v0.7.0은 두 supported
...
`docs/artifacts/v08-compiler-hardening/p161-baseline-inventory.md`가 소유한다.
...
`docs/artifacts/v08-compiler-hardening/p162-parser-recovery.md`가 소유한다. 이어지는 P163에서
...
분류와 validator 경계는
`docs/artifacts/v08-compiler-hardening/p163-invariant-defense.md`가 소유한다. 다음 milestone은
...
`docs/artifacts/v08-compiler-hardening/p164-property-crash-corpus.md`가 소유한다. 다음 milestone은
...
`docs/artifacts/v08-compiler-hardening/p165-performance-reproducibility.md`가 소유한다. 다음
...
`docs/artifacts/v08-compiler-hardening/p166-v08-acceptance.md`가 소유한다. 다음 milestone은
...
`docs/artifacts/v09-language-freeze/p167-normative-inventory.md`가 소유한다. 다음 milestone은
...
`docs/artifacts/v09-language-freeze/p168-version-compatibility.md`가 소유한다. 다음 milestone은
...
`docs/artifacts/v09-language-freeze/p169-conformance-migration.md`가 소유한다. 다음 milestone은
...
`docs/artifacts/v09-language-freeze/p170-representative-dogfood.md`가 소유한다. 다음 milestone은
...
`docs/artifacts/v09-language-freeze/p171-v09-acceptance.md`가 소유한다. 다음 milestone은
...
`docs/artifacts/v09-language-freeze/p172-v1-rc-rehearsal.md`가 소유한다. 다음 milestone은
...
`docs/artifacts/v1-stable-release/acceptance.md`가 소유한다. 이후 변경은
...
`docs/artifacts/v11-streaming-text-io/`. This is a backward-compatible minor release;
...
`docs/SELF_HOSTING.md` and `docs/artifacts/self-hosting-bootstrap/`. B1 subsequently
...
Exact scope and decisions are owned by `docs/artifacts/self-hosting-frontend/`.
...
- [x] 영역별 `--focus`, bounded differential workers와 CI gate deduplication
- [x] selected project test를 위한 deterministic single-C runner와 per-case process isolation
...
- [x] built-in ADT constructor typed IR와 payload move cleanup
...
- [x] nested built-in/user-enum pattern과 recursive owned enum typed IR
- [x] statement-form match arm cleanup과 outer branch move compensation
...
- [x] B2 canonical, publication과 supported-platform CI acceptance

Exact scope and decisions are owned by `docs/artifacts/self-hosting-semantics/`.
...
- [x] string runtime, UTF-8 literal/equality, owned return/local/drop과 full-expression cleanup
- [x] statement `if`/`else`, condition loop, `break`/`continue` C emission
- [x] scalar/owned-control 2-fixture byte identity, native, accounting과 sanitizer gate
- [x] expanded compiler source 725-function typed-IR parity
- [x] type-shape/struct/enum IR metadata와 array/slice/struct C layout
- [x] composite literal, field/index/len, recursive drop helper와 checked bounds rejection
- [x] 3 positive/2 rejection backend fixtures와 expanded compiler source 806-function IR parity
- [x] `Option`/`Result`, inline/owned user ADT definition, constructor와 recursive drop helper
- [x] expression `if`, statement/expression `match`, nested pattern과 span-stable binding environment
- [x] 4 positive/4 rejection backend paths와 expanded compiler source 831-function IR parity
- [x] range/three-clause loop, post-preserving `continue`, loop/range owner cleanup
...
- [x] linked-project `strings.fromInt`/`fromBool` dynamic owned string backend
- [x] 8 positive/4 runtime rejection/0 boundary path와 compiler source 855-function IR parity
- [x] `con`/`mut` pointer ABI, direct borrowed argument와 receiver method backend
- [x] 9 positive/4 runtime rejection/1 function-value boundary와 857-function IR parity
- [x] named function value, callable ABI/drop helper와 indirect call backend
- [x] 10 positive/4 runtime rejection/1 closure boundary와 880-function IR parity
- [x] checked integer negate/divide/remainder와 canonical logical IR kind backend
...
`docs/artifacts/self-hosting-backend/`. Closures and intrinsic function values are
...
`docs/artifacts/self-hosting-fixed-point/`. The fixed-point gate remains parallel to
...
- [x] graph-bridged project `check`/`build`/`run`과 imported generic parity
...
- [x] self-hosted format과 atomic project write/check workflow
- [x] self-hosted test selection과 runner generation
- [x] self-hosted run과 native process workflow
- [x] self-hosted no-flag default와 explicit non-silent Stage0 recovery
...
`docs/artifacts/self-hosting-default-compiler/`. B5 keeps `mlg` as the public command
