...
if [[ $# -ne 1 ]]; then
  echo "usage: scripts/check-standard-strings-runtime.sh <generated-c>" >&2
  exit 2
fi

GENERATED_C="$1"
CLANG_BIN="${CLANG:-clang}"
OUT_DIR="target/mallang/standard-strings-runtime"
EXPECTED_OUTPUT=$'8\n3\n8\ntrue\n1\na||b|\n가/a\n-42\ntrue\n-9223372036854775808\nInvalidData\nfalse'

if [[ ! -f "$GENERATED_C" ]]; then
  echo "missing generated standard strings C: $GENERATED_C" >&2
  exit 1
fi
...
int main(void) {
    if (mallang_live_allocation_count() != 0) {
        fprintf(stderr, "standard string accounting did not start at zero\n");
        return 2;
    }
    if (mallang_example_main() != 0) {
        fprintf(stderr, "standard string example main failed\n");
        return 3;
    }
    if (mallang_live_allocation_count() != 0) {
        fprintf(stderr, "standard string example leaked compiler-owned allocations\n");
        return 4;
    }
    return 0;
}
...
if [[ "$accounting_output" != "$EXPECTED_OUTPUT" ]] || \
  ...
  echo "standard string allocation accounting output mismatch" >&2
...
if [[ "$sanitized_output" != "$EXPECTED_OUTPUT" ]] || \
  ...
  echo "standard string allocation accounting sanitizer mismatch" >&2
...
for ((index = 0; index < 64; index = index + 1)); do
  ...
  if output="$("$binary" 2>"$run_stderr")"; then
    if [[ "$output" != "$EXPECTED_OUTPUT" ]] || [[ -s "$run_stderr" ]]; then
      echo "standard string non-failing injection output mismatch at index $index" >&2
      cat "$run_stderr" >&2
      exit 1
    fi
  ...
  if ! grep -Eq '^mallang runtime error: .*allocation failed$' "$run_stderr"; then
    echo "standard string allocation failure diagnostic mismatch at index $index" >&2
    cat "$run_stderr" >&2
    exit 1
  fi
done
...
if [[ -z "$first_non_failing_index" ]] || [[ "$first_non_failing_index" -eq 0 ]]; then
  echo "standard string allocation failure sweep did not reach a valid boundary" >&2
  exit 1
fi

echo "standard string allocation, failure injection, and sanitizer harness passed"
