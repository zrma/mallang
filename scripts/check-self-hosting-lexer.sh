...
if [[ "$MODE" == "full" ]]; then
  ...
  for test_id in \
    ...
    bootstrap_compiler/ir::LowersNestedMatchPatterns; do
...
echo "self-hosting B2e2c3m $MODE gate passed: parser-corpus=$parser_corpus_count elapsed=$((SECONDS - gate_started))s"
