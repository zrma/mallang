...
if grep -Eiq '[0-9]+개 저장소|[0-9]+ repositories|all repositories|cross-repository .*agent-harness-v1|agent-harness-v1.*cross-repository' AGENTS.md ||
  find docs -type f -name '*.md' -exec grep -Eil '[0-9]+개 저장소|[0-9]+ repositories|all repositories|cross-repository .*agent-harness-v1|agent-harness-v1.*cross-repository' {} + | grep -q .; then
