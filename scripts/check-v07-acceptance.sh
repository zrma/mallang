...
if [[ ! -f "$release_archive" ]]; then
  echo "clean-project release acceptance archive is missing: $release_archive" >&2
  exit 1
fi
...
if [[ ! -x "$mlg" ]]; then
  echo "clean-project release acceptance compiler is missing: $mlg" >&2
  exit 1
fi
...
if "$mlg" fmt --check "$toolkit" >"$work/toolkit-check.stdout" 2>"$work/toolkit-check.stderr"; then
  echo "clean-project release acceptance expected the toolkit project to need formatting" >&2
  exit 1
fi
if [[ -s "$work/toolkit-check.stdout" ]] || \
  ...
  echo "clean-project release acceptance toolkit no-write format check mismatch" >&2
  exit 1
fi

if "$mlg" fmt --check "$workflow" >"$work/workflow-check.stdout" 2>"$work/workflow-check.stderr"; then
  echo "clean-project release acceptance expected the workflow project to need formatting" >&2
  exit 1
fi
...
if [[ -s "$work/workflow-check.stdout" ]] || \
  ...
  echo "clean-project release acceptance workflow no-write format check mismatch" >&2
  exit 1
fi
...
if [[ "$(cat "$work/toolkit-format.stdout")" != "src/library.mlg: formatted" ]] || \
  ...
  echo "clean-project release acceptance toolkit format output mismatch" >&2
  exit 1
fi
...
if [[ -s "$work/workflow-format.stderr" ]] || \
  ...
  echo "clean-project release acceptance workflow format output mismatch" >&2
  exit 1
fi
...
if [[ -s "$work/toolkit-clean.stdout" || -s "$work/toolkit-clean.stderr" || \
  ...
  echo "clean-project release acceptance formatter determinism or idempotence mismatch" >&2
  exit 1
fi
...
if [[ "$(cat "$work/toolkit-compile.stdout")" != "$toolkit: ok" ]] || \
  ...
  echo "clean-project release acceptance project check output mismatch" >&2
  exit 1
fi
...
if [[ "$(cat "$work/test.stdout")" != "$expected_test" ]] || [[ -s "$work/test.stderr" ]]; then
  echo "clean-project release acceptance project test output mismatch" >&2
...
if [[ "$(cat "$work/build.stdout")" != "$binary" ]] || \
  ...
  echo "clean-project release acceptance native build output mismatch" >&2
  exit 1
fi
if [[ "$("$binary" 2>"$work/binary.stderr")" != "42" ]] || \
  [[ -s "$work/binary.stderr" ]]; then
  echo "clean-project release acceptance built program output mismatch" >&2
  exit 1
fi
if [[ "$("$mlg" run "$workflow" 2>"$work/run.stderr")" != "42" ]] || \
  [[ -s "$work/run.stderr" ]]; then
  echo "clean-project release acceptance installed compiler run output mismatch" >&2
  exit 1
fi

echo "clean-project format, check, test, build, install, and run release acceptance passed"
